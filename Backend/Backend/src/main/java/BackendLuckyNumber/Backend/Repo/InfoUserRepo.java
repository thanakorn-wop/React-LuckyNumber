...
public interface InfoUserRepo extends JpaRepository<InfoUserModal,String>   {
	...
	@Query(value = " SELECT * FROM info_user  WHERE id = ?1 and nickname = ?2 and status_transfer = 'N' ORDER by date DESC LIMIT 1",nativeQuery = true)
	InfoUserModal findLastInfoUser(String id,String nickname);
